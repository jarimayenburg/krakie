...
async fn main() {
    let tileset = load_texture("assets/tilesets/test.png").await.unwrap();
    tileset.set_filter(FilterMode::Nearest);

    let tiled_map_json = load_string("assets/maps/test.json").await.unwrap();
    let tiled_map = tiled::load_map(&tiled_map_json, &[("test.png", tileset)], &[]).unwrap();
